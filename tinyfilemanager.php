    ...
    if ($new != '' && $new != '..' && $new != '.') {
        ...
        if ($_GET['type'] == "file") {
            if (!file_exists($path . '/' . $new)) {
                @_fopen($path . '/' . $new, 'w') or die('Cannot open file:  ' . $new);
...
function _fopen($filename,$mode, $use_include_path = FALSE, $context = NULL) {
    global $global;
    if(preg_match("/avideo.com/", $global['aVideoStorageURL'])){
        return false;
    }
